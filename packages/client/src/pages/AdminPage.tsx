    ...
    <AnimatedPageContainer>
      <div className="mb-8 flex items-start justify-between">
        ...
        <div className="flex items-center gap-3 bg-neutral-800/40 backdrop-blur-xl border border-neutral-700/50 rounded-lg px-4 py-2.5">
          ...
          <span className="text-sm text-neutral-300">Expert Mode</span>
          <button
            onClick={toggleExpertMode}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              expertMode ? 'bg-neutral-500' : 'bg-neutral-600'
            }`}
          ...
          <span className={`text-xs font-medium ${expertMode ? 'text-neutral-400' : 'text-neutral-500'}`}>
            {expertMode ? 'ON' : 'OFF'}
          </span>
        </div>
      </div>
      ...
      {activeTab === 'repositories' && (
        <div className={designTokens.animations.fadeIn}>
          ...
          >
            {error && (
              <div className="mb-4 p-4 bg-neutral-700/10 border border-neutral-700/30 rounded-xl">
            ...
            {refreshResults && (
              <div className="mb-6 p-4 bg-neutral-500/10 border border-neutral-500/30 rounded-xl">
                ...
                <p className="text-sm mb-3 text-neutral-300">{refreshResults.message}</p>
                    ...
                    {refreshResults.results.map((result, i) => (
                      <div
                        ...
                        className={`p-2 rounded-lg ${
                          result.success ? 'bg-neutral-500/10' : 'bg-neutral-700/10'
                        }`}
                      >
                        ...
                        {result.error && (
                          <p className="text-xs text-neutral-500 mt-1">{result.error}</p>
                        )}
                      </div>
                    ))}
                  </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10 text-left">
                      <th className="pb-3 font-semibold text-neutral-300">Project</th>
                      <th className="pb-3 font-semibold text-neutral-300">GitLab Repository</th>
                      <th className="pb-3 font-semibold text-neutral-300">Version</th>
                      <th className="pb-3 font-semibold text-neutral-300">Last Updated</th>
                    </tr>
                  </thead>
                  <tbody>
                    {repositories.map((repo) => (
                      <tr key={repo.id} className="border-b border-white/5">
                        ...
                        <td className="py-3">
                          <span className="px-2 py-1 bg-white/10 rounded text-xs font-mono text-neutral-300">
            ...
            {opsResult && (
              <div className={`mb-4 p-4 rounded-xl ${
                opsResult.success
                  ...
                  : 'bg-neutral-700/10 border border-neutral-700/30'
              }`}>
                ...
                <p className="text-sm text-neutral-300">{opsResult.message}</p>
              </div>
            )}
          ...
          <div className="grid grid-cols-4 gap-4 my-6">
            {[
              ...
              <div key={idx} className="bg-white/5 backdrop-blur-md rounded-xl border border-white/10 p-4 hover:bg-white/10 transition-all">
                ...
                {stat.subtitle && <div className="text-xs text-neutral-500 mt-1">{stat.subtitle}</div>}
              </div>
            ))}
          </div>
            ...
            {usageLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10 text-left">
                      <th className="pb-3 font-semibold text-neutral-300">Date</th>
                      <th className="pb-3 font-semibold text-neutral-300">Provider</th>
                      <th className="pb-3 font-semibold text-neutral-300">Goal</th>
                      <th className="pb-3 font-semibold text-neutral-300">Phase</th>
                      <th className="pb-3 font-semibold text-neutral-300">Tokens</th>
                      <th className="pb-3 font-semibold text-neutral-300">CI Time</th>
                      <th className="pb-3 font-semibold text-neutral-300">Cost</th>
                    </tr>
                  </thead>
                  <tbody>
                    {usageMetrics.map((metric) => {
                      ...
                      return (
                        <tr key={metric.id} className="border-b border-white/5">
                          ...
                          <td className="py-3 text-neutral-400 text-xs">{metric.operation_phase}</td>
