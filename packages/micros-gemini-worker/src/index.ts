/**
 * Gemini Worker Microservice
 * Consumes tasks from RabbitMQ and executes them using Google Gemini
 */
...
import { executeEvaluation, executeImplementation } from './utils/gemini-executor'
...
const logger = createLogger({ namespace: 'gemini-worker' })

// Environment configuration
const RABBITMQ_URL = process.env.RABBITMQ_URL || 'amqp://localhost'
const WORKER_NAME = process.env.WORKER_NAME || 'gemini-worker'
const MAX_CONCURRENCY = parseInt(process.env.MAX_CONCURRENCY || '3', 10)
const GOOGLE_API_KEY = process.env.GOOGLE_API_KEY || ''
const WORKER_VERSION = '1.0.0'

/**
 * Get Google API key from task context or environment
 */
function getApiKey(task: WorkerTaskMessage): string {
  const aiProvider = task.context.aiProvider || {}
  const googleConfig = aiProvider.google || {}
  const apiKey = googleConfig.apiKey || GOOGLE_API_KEY
...
function getModel(task: WorkerTaskMessage): string {
  const aiProvider = task.context.aiProvider || {}
  const googleConfig = aiProvider.google || {}
  return googleConfig.model || 'gemini-1.5-pro'
}
...
/**
 * Execute task evaluation using Gemini
 */
async function evaluateTask(task: WorkerTaskMessage): Promise<any> {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'gemini-eval-'))
...
/**
 * Execute task implementation using Gemini
 */
async function implementTask(task: WorkerTaskMessage): Promise<any> {
  ...
  // Create temporary directory for workspaces
  const tempDir = await mkdtemp(join(tmpdir(), 'gemini-impl-'))
  logger.info(`Created temp directory: ${tempDir}`)
...
Please implement this task in the appropriate workspace(s).`
    ...
    for (const ws of workspaces) {
      try {
        ...
        // Configure git
        await exec(`cd "${ws.path}" && git config user.name "ADI Gemini Worker"`)
...
async function main() {
  logger.info('===========================================')
  logger.info('  Gemini Worker Microservice')
  logger.info('===========================================')
  ...
  if (!GOOGLE_API_KEY) {
    logger.warn('⚠️  GOOGLE_API_KEY not set - tasks must provide their own API keys')
  }
  ...
  try {
    ...
    logger.error('Failed to start Gemini worker:', error)
    process.exit(1)
  }
}
