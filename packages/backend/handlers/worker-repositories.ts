...
import { GITLAB_HOST, GITLAB_TOKEN, GITLAB_USER, ENCRYPTION_KEY } from '@backend/config'
...
export const createWorkerRepositoryRoutes = (sql: Sql) => {
  return new Hono()
    ...
    .get('/:id', zValidator('param', idParamSchema), async (c) => {
      ...
      const repo = await queries.findWorkerRepositoryById(sql, id)
    ...
    .patch('/:id', zValidator('param', idParamSchema), zValidator('json', updateWorkerRepositorySchema), authMiddleware, async (c) => {
      const { id } = c.req.valid('param')
      const body = c.req.valid('json')
      const repo = await queries.updateWorkerRepository(sql, id, body)

      return c.json(repo)
    })
    .delete('/:id', zValidator('param', idParamSchema), authMiddleware, async (c) => {
      const { id } = c.req.valid('param')
      await queries.deleteWorkerRepository(sql, id)
    ...
    .get('/by-project/:projectId', zValidator('param', projectIdParamSchema), async (c) => {
      ...
      const repo = await queries.findWorkerRepositoryByProjectId(sql, projectId)
    ...
    try {
      ...
      const workerRepo = await queries.findWorkerRepositoryById(sql, id)
      ...
      // Update database
      const updatedRepo = await queries.updateWorkerRepository(sql, id, {
