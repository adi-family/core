 ...
 * Consumes tasks from RabbitMQ and executes them using Claude
 */
...
const logger = createLogger({ namespace: 'claude-worker' })

// Environment configuration
const RABBITMQ_URL = process.env.RABBITMQ_URL || 'amqp://localhost'
const WORKER_NAME = process.env.WORKER_NAME || 'claude-worker'
const MAX_CONCURRENCY = parseInt(process.env.MAX_CONCURRENCY || '3', 10)
const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY || ''
const WORKER_VERSION = '1.0.0'

/**
 * Get Anthropic API key from task context or environment
 */
function getApiKey(task: WorkerTaskMessage): string {
  const aiProvider = task.context.aiProvider || {}
  const anthropicConfig = aiProvider.anthropic || {}
  const apiKey = anthropicConfig.apiKey || ANTHROPIC_API_KEY
...
function getModel(task: WorkerTaskMessage): string {
  const aiProvider = task.context.aiProvider || {}
  const anthropicConfig = aiProvider.anthropic || {}
  return anthropicConfig.model || 'claude-sonnet-4-5-20250929'
}
...
/**
 * Execute task evaluation using Claude
 */
async function evaluateTask(task: WorkerTaskMessage): Promise<any> {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'claude-eval-'))
...
/**
 * Execute task implementation using Claude
 */
async function implementTask(task: WorkerTaskMessage): Promise<any> {
  ...
  // Create temporary directory for workspaces
  const tempDir = await mkdtemp(join(tmpdir(), 'claude-impl-'))
  logger.info(`Created temp directory: ${tempDir}`)
...
Please implement this task in the appropriate workspace(s).`
    ...
    for (const ws of workspaces) {
      try {
        ...
        // Configure git
        await exec(`cd "${ws.path}" && git config user.name "ADI Claude Worker"`)
...
const handler: TaskHandler = {
  ...
  async onCancel(sessionId: string): Promise<void> {
    ...
    // This should kill any running Claude processes for this session
  }
}
...
async function main() {
  logger.info('===========================================')
  logger.info('  Claude Worker Microservice')
  logger.info('===========================================')
  ...
  if (!ANTHROPIC_API_KEY) {
    logger.warn('⚠️  ANTHROPIC_API_KEY not set - tasks must provide their own API keys')
  }
