...
// packages/worker/templates/2025-10-18-01/worker-scripts/evaluation-pipeline.ts
...
function discoverWorkspaces() {
  logger2.info("\uD83D\uDD0D Checking for cloned workspaces...");
  logger2.info(`   Environment variables:`);
  logger2.info(`   - WORKSPACE_COUNT: ${process.env.WORKSPACE_COUNT || "not set"}`);
  logger2.info(`   - WORKSPACE_DIRS: ${process.env.WORKSPACE_DIRS ? `${process.env.WORKSPACE_DIRS.substring(0, 100)}...` : "not set"}`);
  logger2.info(`   - WORKSPACE_NAMES: ${process.env.WORKSPACE_NAMES || "not set"}`);
  logger2.info(`   - WORKSPACE_BRANCHES: ${process.env.WORKSPACE_BRANCHES || "not set"}`);
  logger2.info(`   - FILE_SPACES: ${process.env.FILE_SPACES ? "set" : "not set"}`);
  ...
  if (workspaceCount === 0 || workspaceDirs.length === 0) {
    logger2.error("\u274C No workspaces found!");
    logger2.error("   This usually means:");
    logger2.error("   1. No FILE_SPACES environment variable was passed to the pipeline");
    logger2.error("   2. The clone-workspace.sh script did not run");
    logger2.error("   3. The clone-workspace.sh script failed");
    logger2.error("");
    logger2.error("   Please ensure:");
    logger2.error("   - Task has file spaces configured");
    logger2.error("   - FILE_SPACES variable is passed from pipeline-executor");
    logger2.error("   - clone-workspace.sh script executed successfully in CI");
    throw new Error("No workspaces available for evaluation. Evaluation requires at least one workspace with code to analyze.");
  }
  logger2.info(`\uD83D\uDCE6 Found ${workspaceCount} cloned workspace(s)`);
...
function validateWorkspaces(workspaceDirs, workspaceNames, workspaceBranches, workspaceCount) {
  ...
  for (let i = 0;i < workspaceDirs.length; i++) {
    ...
    if (!dir) {
      logger2.warn(`   \u26A0\uFE0F  Workspace ${i} has no directory, skipping`);
      continue;
    }
    logger2.info(`   Checking workspace ${i + 1}/${workspaceCount}: ${name}`);
    logger2.info(`     Directory: ${dir}`);
    logger2.info(`     Branch: ${branch}`);
    if (existsSync2(dir)) {
      validWorkspaces.push({ dir, name, branch });
      logger2.info(`     \u2713 Directory exists`);
    } else {
      logger2.error(`     \u274C Directory not found: ${dir}`);
    }
  }
  if (validWorkspaces.length === 0) {
    logger2.error("\u274C No valid workspace directories found!");
    logger2.error(`   Expected directories:`);
    workspaceDirs.forEach((dir, i) => {
      logger2.error(`   - ${workspaceNames[i] || `workspace-${i}`}: ${dir}`);
    });
    throw new Error("All workspace directories are missing or inaccessible");
  }
  logger2.info(`\u2713 ${validWorkspaces.length} valid workspace(s) ready for evaluation`);
  return validWorkspaces;
}
function buildCodebaseInfo(validWorkspaces) {
  if (validWorkspaces.length === 1) {
    const ws = validWorkspaces[0];
    logger2.info(`   Using single workspace: ${ws.name}`);
    return `Codebase cloned from repository (branch: ${ws.branch}) at: ${ws.dir}`;
  }
  logger2.info(`   Using multiple workspaces:`);
  validWorkspaces.forEach((ws) => {
    logger2.info(`     - ${ws.name} (${ws.branch}): ${ws.dir}`);
  });
...
function prepareClaudeEnvironment(env) {
  ...
  if (process.env.PROXY_HOST && process.env.PROXY_USER && process.env.PROXY_PASS) {
    ...
    logger2.info(`\u2713 Configured standard proxy environment variables for Claude Code`);
  }
  return claudeEnv;
}
function getClaudePath() {
  ...
  logger2.info(`Using Claude executable: ${claudePath}`);
  if (!existsSync2(claudePath)) {
    logger2.error(`\u274C Claude executable not found at: ${claudePath}`);
    logger2.info("Checking alternative locations...");
    const altPath = resolve(__dirname2, "../node_modules/.bin/claude");
    logger2.info(`Alternative path: ${altPath} - exists: ${existsSync2(altPath)}`);
  } else {
    logger2.info(`\u2713 Claude executable found at: ${claudePath}`);
  }
  return claudePath;
}
function determineWorkingDirectory(workspaceCount, workspaceDirs) {
  logger2.info("\uD83D\uDCC2 Determining Claude Code working directory...");
  logger2.info(`   Workspace count: ${workspaceCount}`);
  logger2.info(`   Workspace dirs length: ${workspaceDirs.length}`);
  if (workspaceCount === 0 || workspaceDirs.length === 0) {
    logger2.info(`   Strategy: Fallback (no workspaces)`);
    logger2.info(`   Selected: ..`);
    return "..";
  }
  const firstDir = workspaceDirs[0];
  if (!firstDir) {
    logger2.warn(`   \u26A0\uFE0F  First workspace directory is undefined, using fallback`);
    return "..";
  }
  if (workspaceCount === 1) {
    logger2.info(`   Strategy: Single workspace`);
    logger2.info(`   Selected: ${firstDir}`);
    return firstDir;
  }
  const parentDir = firstDir.replace(/\/workspace-\d+$/, "");
  logger2.info(`   Strategy: Multiple workspaces`);
  logger2.info(`   First workspace: ${firstDir}`);
  logger2.info(`   Extracted parent: ${parentDir}`);
  logger2.info(`   Selected: ${parentDir}`);
  return parentDir;
}
async function executeClaudeAgent(prompt, claudeEnv, claudePath, workingDir, agenticStart) {
  if (!existsSync2(workingDir)) {
    throw new Error(`Working directory not found: ${workingDir}`);
  }
  logger2.info(`   \u2713 Working directory exists`);
  logger2.info("");
  logger2.info(`\uD83D\uDCCB Claude Code query options:`);
  logger2.info(`  - permissionMode: acceptEdits`);
  logger2.info(`  - claudePath: ${claudePath}`);
  logger2.info(`  - cwd: ${workingDir}`);
  logger2.info(`  - allowedTools: Bash, Read, Glob, Grep, Write`);
  logger2.info(`  - ANTHROPIC_API_KEY set: ${!!claudeEnv.ANTHROPIC_API_KEY}`);
  logger2.info(`  - HTTP_PROXY set: ${!!claudeEnv.HTTP_PROXY}`);
  logger2.info(`  - HTTPS_PROXY set: ${!!claudeEnv.HTTPS_PROXY}`);
  const iterator = query({
    ...
    options: {
      ...
      stderr: (data) => {
        logger2.error(`[Claude Code stderr] ${data}`);
      }
    }
  });
  logger2.info("\u2713 Query iterator created, starting iteration...");
  let iterations = 0;
  for await (const chunk of iterator) {
    iterations++;
    logger2.info(`\uD83D\uDCE5 Iteration ${iterations}: chunk type = ${chunk.type}`);
    if (chunk.type === "system") {
      logger2.info(`[System] subtype=${chunk.subtype}`);
      if ("message" in chunk) {
        logger2.info(`[System] message=${JSON.stringify(chunk.message)}`);
      }
    }
    if (chunk.type === "assistant") {
      logger2.info(`[Assistant] ${JSON.stringify(chunk.message)}`);
    }
    if (chunk.type === "stream_event") {
      logger2.info(`[Stream] event=${JSON.stringify(chunk.event)}`);
    }
    if (chunk.type === "result") {
      const cost = chunk.total_cost_usd || 0;
      logger2.info(`\u2713 Claude Agent completed - Cost: $${cost.toFixed(4)}, Iterations: ${iterations}`);
      const agenticUsage = {
        provider: "anthropic",
        model: "claude-sonnet-4-5",
        goal: "evaluation",
  ...
  logger2.info("\u2713 Claude Agent SDK execution completed");
}
async function readEvaluationResults() {
  logger2.info("Checking results directory...");
  try {
    const files = await readdir("../results");
    logger2.info(`Files in results directory: ${files.join(", ")}`);
  } catch {
...
async function agenticEvaluation(task, env) {
  logger2.info("\uD83D\uDD2C Phase 2: Running deep agentic evaluation...");
  ...
  try {
    logger2.info("\uD83E\uDD16 Starting Claude Agent SDK for evaluation...");
    ...
    logger2.error(`\u274C Claude Agent SDK error: ${errorMsg}`);
    if (errorStack) {
      logger2.error(`Stack trace:
${errorStack}`);
    }
...
async function main() {
  logger2.info("\uD83E\uDD16 Advanced Evaluation Pipeline Started");
  logger2.info("\u2139\uFE0F  Simple evaluation already completed in microservice - running advanced evaluation only");
  try {
    ...
    logger2.info(`Execution ID: ${executionId}`);
    ...
    logger2.info("\uD83D\uDD2C Running advanced agentic evaluation...");
    await agenticEvaluation(task, { ANTHROPIC_API_KEY: env.ANTHROPIC_API_KEY });
    logger2.info("\u2705 Advanced evaluation pipeline completed successfully");
  } catch (error) {
    logger2.error("\u274C Advanced evaluation pipeline failed:", error);
    try {
      await mkdir("../results", { recursive: true });
      await writeFile("../results/error.json", JSON.stringify({
        error: error instanceof Error ? error.message : String(error),
        timestamp: new Date().toISOString(),
        stack: error instanceof Error ? error.stack : undefined
      }, null, 2), "utf-8");
      logger2.info("\uD83D\uDCDD Error file written to results/error.json");
    } catch (writeError) {
      logger2.error("Failed to write error file:", writeError);
    }
    try {
      ...
      if (session.task_id) {
        ...
        logger2.info("Task evaluation status updated to failed");
      }
    } catch (statusError) {
      logger2.error("Failed to update task evaluation status to failed:", statusError);
    }
    throw error;
  }
}
