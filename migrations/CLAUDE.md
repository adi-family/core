migration-management, golang-migrate, database-schema, automation

- Uses golang-migrate for database migrations
- Flat structure with timestamp-based naming
- Runs automatically via Docker on compose up
...
- Timestamp format: 20251010223045 (year, month, day, hour, minute, second)
- Use `./create_migration.sh migration_name` to generate new migrations
- Database: postgres://postgres:postgres@postgres:5432/postgres (internal), localhost:5436 (external)

## Available Commands
- `up [N]` - Apply all or N up migrations
- `down [N]` or `down -all` - Rollback N or all migrations
